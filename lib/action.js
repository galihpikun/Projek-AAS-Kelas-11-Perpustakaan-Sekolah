...
export async function approveBorrow(formData) {
...
export async function confirmReturn(formData) {
  ...
  await connection.execute(`
    ...
    SET 
      ...
      return_date = NOW(),
      fine_amount = CASE
        WHEN NOW() > due_date THEN TIMESTAMPDIFF(DAY, due_date, NOW()) * 1000
        ELSE 0
      END
    WHERE borrow_id = ?
  `, [idBorrow]);
...
export async function deleteUserProfile() {
  ...
  await connection.execute(`DELETE FROM users WHERE user_id = ?`, [user.id]);
...
export async function editUsername (formData) {
  ...
  await connection.execute(`update users set username = ? where user_id = ?`, [username, user.id])
...
export async function editEmail (formData) {
  ...
  await connection.execute(`update users set email = ? where user_id = ?`, [email, user.id])
...
export async function editPassword (formData) {
  ...
  await connection.execute(`update users set password = ? where user_id = ?`, [password, user.id])
